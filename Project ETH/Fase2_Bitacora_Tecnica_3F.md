...
- **Fallo en etapa de ejecución (deadlock del multisig): recuperación fuera de alcance del MVP.** Si un proyecto se financia y sus fondos pasan a `Ejecucion`, pero el plazo de ejecución vence sin reunir las 2 firmas, `vencerPlazoEjecucion` marca el proyecto `Fallido` para dejar constancia on-chain. Sin embargo, la devolución Pull-over-Push vive en los Pools y solo cubre los fallos *en financiación* (donde el dinero sigue en el Pool); en este caso los fondos quedan en custodia de `Ejecucion` sin ruta de salida. Es un caso borde (requiere que un proyecto financiado nunca alcance las 2 firmas en plazo). Se difiere a v2, donde se agregaría una función de recuperación por `MUNICIPIO_ROLE` o un reembolso proporcional desde `Ejecucion`. Decisión consciente para no inflar el MVP.
